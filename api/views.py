...
def get_products_by_category(request, id):
    ...
    if id == 6:
        prod = Product.objects.filter(category="PHONES")

        categories_json = [category.to_json() for category in prod]
        return JsonResponse(categories_json, safe=False)
    elif id == 7:
        prod = Product.objects.filter(category="Laptops")

        categories_json = [category.to_json() for category in prod]
        return JsonResponse(categories_json, safe=False)
    elif id == 8:
        prod = Product.objects.filter(category="TV")

        categories_json = [category.to_json() for category in prod]
        return JsonResponse(categories_json, safe=False)
    elif id == 9:
        prod = Product.objects.filter(category="PC")
